...
vector<int> help_cover(pair<float, float>& target,
		  ...
		  vector<pair<float, float>>& intervals)
{
    ...
    {
	intervals_copy.push_back(vector<float>{
		intervals[i].first, intervals[i].second, (float) i});
    }

    sort(intervals_copy.begin(), intervals_copy.end(), [] (vector<float>& vec1,
							   vector<float>& vec2)
	 {
	     If (vec1[0] == vec2[0])
