...
namespace dalg
{
    ...
    {
	//If lines are parallel return monostate
	...
	//Two points p1,p2 representing l1
	Vec2d<T> p1{l1.p0};
	Vec2d<T> p2{l1.p0 + l1.u};

	//Two points p3,p4 representing l2
	Vec2d<T> p3{l2.p0};
	Vec2d<T> p4{l2.p0 + l2.u};


	//Create vector x1 and y1 containing both x values and y values  of l1 respectively
	Vec2d<T> x1{p1.x, p2.x};
	Vec2d<T> y1{p1.y, p2.y};

	//Create vector x2 and y2 containing both x values and y values  of l2 respectively
	Vec2d<T> x2{p3.x, p4.x};
	Vec2d<T> y2{p3.y, p4.y};

	Vec2d<T> v1{0,0};

	T px =
 	    cross( Vec2d<T>{cross(x1, y1), cross(x1, v1)},
		   Vec2d<T>{cross(x2, y2), cross(x2, v1)})
	    /
	    cross( Vec2d<T>{cross(x1, v1), cross(y1, v1)},
		   Vec2d<T>{cross(x2, v1), cross(y2, v1)});

	T py =
	    cross( Vec2d<T>{cross(x1, y1), cross(y1, v1)},
		   Vec2d<T>{cross(x2, y2), cross(y2, v1)})
	    /
	    cross( Vec2d<T>{cross(x1, v1), cross(y1, v1)},
		   Vec2d<T>{cross(x2, v1), cross(y2, v1)});

	Vec2d<T> result{px, py};
